...
The EFS file system for this backend was created manually in the AWS console, with its own
security group, which has a single inbound rule, allowing access from the security
group assigned to the launched EC2 instances. It is in a single availabilty zone to
minimise costs during testing.
