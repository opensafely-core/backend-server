#!/bin/bash
# This script is meant to be run in git-bash on Windows, and builds a hyper-v
# image.
#
# It uses multipass to do the leg work of actually creating a VM:
# https://multipass.run/
#
# It is not fully automated, as Windows has no easy equivalent of sudo that
# I could figure out, and the final commands require running as Administrator.
# So, we launch an Administrator powershell and print some commands for you to
# copy/paste into it.
set -euo pipefail
name=opensafely-tpp
...
#
# Note: this is a Gen 1 instance, as that's what multipass creates.  Ideally,
# we use a Gen 2 instance simply to be more up to date, but there's no
# compelling benefits for our use case. Gen 1 supports up to 64 cores and 1TB
# of RAM, which is enough for current hardware.
#
...
#
# Note: the defaults are 1 cpu, 1G ram, and 5G disk. These will need changing
...
# kernel modules and services.  Note: HWE version is important for a more
# recent kernel, which has better Hyper-V guest support:
...
# Note: multipass mount is hella weird, so we just copy the current directory
# to the host.
tarball=../current.tar
rm -f "$tarball"
...
echo
echo "To export $name VM image to $path, run following in Admin powershell (already copied to clipboard)"
echo "$remove_cmd"
echo "$export_cmd"
powershell -Command Set-Clipboard -Value "${remove_cmd}; ${export_cmd};"
